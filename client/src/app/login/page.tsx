...
const LoginForm = () => {
        ...
        dispatch(
          setCredentials({
            user: {
              ...
              role: (data.user as any).role || "operador",
              image: data.user.image,
              language: (data.user as any).language || "Português (BR)",
            },
          }),
        );
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4 transition-colors">
      <section className="w-full max-w-md bg-white dark:bg-gray-800 shadow-[0_4px_20px_rgb(0,0,0,0.02)] rounded-2xl border border-gray-100 dark:border-gray-700/60 p-8 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative flex items-center">
              <Mail
                className="absolute left-3.5 text-gray-400 w-4 h-4"
                aria-hidden="true"
              />
              <input
                id="email"
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link
                ...
                className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 hover:underline transition-colors"
              >
                Esqueceu a senha?
              </Link>
            </div>
            <div className="relative flex items-center">
              ...
              <button
                ...
                className="absolute right-3.5 p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"
              >
                {verSenha ? (
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider py-3 rounded-xl transition-all shadow-sm hover:shadow-md ${isLoading ? "opacity-60 cursor-not-allowed" : "active:scale-[0.99]"}`}
          >
            {isLoading ? (
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-2.5 bg-white dark:bg-transparent border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/40 font-bold text-xs py-3 rounded-xl transition-all active:scale-[0.99] disabled:opacity-60"
        >
        ...
        <footer className="text-center text-xs pt-2">
